...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="table">
		<xsl:variable name="isFormulaTable">
			<xsl:choose>
				<xsl:when
					test="contains(@table-template-name, 'formula') or contains(@table-template-name, 'Formula')">
				...
				<xsl:choose>
					...
					<xsl:when test="$isFormulaTable = 'true'">
						...
						<xsl:choose>
							...
							<xsl:when
								test="number($tableWidth) >= 30 and number($tableWidth) &lt; 50">
								<xsl:attribute name="width">
									<xsl:text>50%</xsl:text>
								</xsl:attribute>
							</xsl:when>
							<xsl:when
								test="number($tableWidth) >= 50 and number($tableWidth) &lt; 70">
								<xsl:attribute name="width">
	...
	<xsl:template match="//thead//entry">
		<td>
			...
			<xsl:if
				test="@colname = 'I50' or @colname = 'I46' or @colname = 'I47' or @colname = 'I48'">
	...
	<xsl:template match="//tbody//entry">
		...
		<td>
			...
			<xsl:if
				test="contains(ancestor::table/@table-template-name, 'Formula') and position() = 1">
				<xsl:attribute name="width">
			...
			<span>
				...
				<xsl:choose>
					<xsl:when
						test="ancestor::quoted-block and $isDutyTable = 'yes' and not(ancestor::table/@table-type = '9-Duty-Suspension')">
						<xsl:if test="count(preceding-sibling::entry) = 0">
							<xsl:if test="count(../preceding-sibling::row) = 0">
								<xsl:if
					...
					<xsl:when
						test="ancestor::quoted-block and $isDutyTable = 'yes' and ancestor::table/@table-type = '9-Duty-Suspension'">
				...
				<xsl:if test="ancestor::quoted-block and ancestor::quoted-block/after-quoted-block">
					...
					<xsl:variable name="isFormulaTable">
						<xsl:choose>
							<xsl:when
								test="contains(ancestor::table/@table-template-name, 'formula') or contains(ancestor::table/@table-template-name, 'Formula')">
								<xsl:text>true</xsl:text>
							</xsl:when>
							<xsl:otherwise>false</xsl:otherwise>
						</xsl:choose>
	...
	<xsl:template name="isFirstNonemptyColumn">
		...
		<xsl:choose>
			...
			<xsl:when
				test="$isFormulaTable = 'true' and (parent::row/preceding-sibling::*[1]/child::*[1]/text() != '' or parent::row/preceding-sibling::*[1]/child::*[1]/child::*[1]/text() != '')">
				<!-- not the first column -->
				<xsl:text>no</xsl:text>
			</xsl:when>
			<xsl:when test="$isFormulaTable = 'true'">
			...
			<!-- not formula tables -->
			<xsl:when
				test="normalize-space(.) = '' and normalize-space(parent::row/preceding-sibling::row/entry) != ''">
				<xsl:text>no</xsl:text>
			</xsl:when>
	...
	<xsl:template name="isLasttNonemptyColumn">
		...
		<xsl:choose>
			...
			<xsl:when
				test="$isFormulaTable = 'true' and (parent::row/following-sibling::*[position() = last()]/child::*[position() = last()]/text() != '' or parent::row/following-sibling::*[position() = last()]/child::*[position() = last()]/child::*[position() = last()]/text() != '')">
				<!-- not the first column -->
				<xsl:text>no</xsl:text>
			</xsl:when>
			<xsl:when test="$isFormulaTable = 'true'">
			...
			<!-- not formula tables -->
			<xsl:when
				test="normalize-space(.) = '' and normalize-space(./following-sibling::row/entry) != ''">
				<xsl:text>no</xsl:text>
			</xsl:when>
			<xsl:otherwise>
				<xsl:text>yes</xsl:text>
	...
	<xsl:template name="calcFormulaTableWidth">
		...
		<xsl:choose>
			<!-- 1st >= 2nd and 1st >= 3rd -->
			<xsl:when
				test="number($firstRowLength) >= number($secondRowLength) and number($firstRowLength) >= number($thirdRowLength)">
				<xsl:value-of select="$firstRowLength"/>
			</xsl:when>
			<!--2nd >= 1st and 2nd >= 3rd -->
			<xsl:when
				test="number($secondRowLength) >= number($firstRowLength) and number($secondRowLength) >= number($thirdRowLength)">
				<xsl:value-of select="$secondRowLength"/>
			</xsl:when>
			<!--3rd >= 1st and3rd >= 2nd -->
			<xsl:when
				test="number($thirdRowLength) >= number($firstRowLength) and number($thirdRowLength) >= number($secondRowLength)">
	...
	<xsl:template name="theRowLength">
		...
		<xsl:value-of
			select="number($firstCellLength) + number($secondCellLength) + number($thirdCellLength)"/>
	...
	<xsl:template name="singleCellLenght">
		...
		<xsl:choose>
			<xsl:when
				test="count(tgroup/tbody/row[number($aRowNumber)]/entry[number($aCellNumber)]/child::*) = 2">
				<xsl:value-of
					select="string-length(tgroup/tbody/row[$aRowNumber]/entry[$aCellNumber]/child::*[1]/text()) + string-length(tgroup/tbody/row[$aRowNumber]/entry[$aCellNumber]/child::*[2]/text())"
				/>
			</xsl:when>
			<xsl:when
				test="count(tgroup/tbody/row[number($aRowNumber)]/entry[number($aCellNumber)]/child::*) = 3">
				<xsl:value-of
					select="string-length(tgroup/tbody/row[$aRowNumber]/entry[$aCellNumber]/child::*[1]/text()) + string-length(tgroup/tbody/row[$aRowNumber]/entry[$aCellNumber]/child::*[2]/text()) + string-length(tgroup/tbody/row[$aRowNumber]/entry[$aCellNumber]/child::*[3]/text())"
				/>
			</xsl:when>
			<xsl:when
				test="count(tgroup/tbody/row[number($aRowNumber)]/entry[number($aCellNumber)]/child::*) >= 4">
				<xsl:value-of
					select="string-length(tgroup/tbody/row[$aRowNumber]/entry[$aCellNumber]/child::*[1]/text()) + string-length(tgroup/tbody/row[$aRowNumber]/entry[$aCellNumber]/child::*[2]/text()) + string-length(tgroup/tbody/row[$aRowNumber]/entry[$aCellNumber]/child::*[3]/text()) + string-length(tgroup/tbody/row[$aRowNumber]/entry[$aCellNumber]/child::*[4]/text())"
				/>
			</xsl:when>
			<xsl:otherwise>
				<xsl:value-of
					select="string-length(tgroup/tbody/row[$aRowNumber]/entry[$aCellNumber]/text())"
				/>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
